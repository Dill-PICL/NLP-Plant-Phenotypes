...
if (file.exists(file_path_composition_files)){
  input_name <- list.files(pattern = "token_frequencies.csv")
  input_path <- paste(file_path_composition_files, "/", input_name, sep = "")
}
output_path <- paste(parent_directory, "/figs/vocabs_compared.pdf", sep = "")


# The input and output files that this script uses and creates.
#input_path <- "/Users/irbraun/phenologs-with-oats/outputs/composition_01_13_2021_h10m34s56_3320/token_frequencies.csv"
...
ggsave(output_path, plot=last_plot(), device="pdf", path=NULL, scale=1, width=20, height=14, units=c("cm"), dpi=300, limitsize=TRUE)
