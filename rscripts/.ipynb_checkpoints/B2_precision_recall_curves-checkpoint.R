...
input_path <- "/work/triffid/prasanth/reorganizing-irb-scripts/plant-phenotypes-nlp/outputs/stacked_05_02_2023_h14m52s44_8825/stacked_precision_recall_curves.csv"
...
#   output_composition_name <- list.files(pattern = "stacked")
...
#   input_name <- list.files(pattern = "stacked_precision_recall_curves.csv")
#   input_path <- paste(file_path_composition_files, "/", input_name, sep = "")
# }

# output_dir <- paste(parent_directory, "/figs/precision_recall_curves/", sep = "")

# The input and output files that this script uses and creates.
# input_path <- "/Users/irbraun/phenologs-with-oats/outputs/stacked_01_19_2021_h13m44s23_5388_plants/stacked_precision_recall_curves.csv"
...
make_and_save_figure <- function(df, baselines, output_path){
  ...
  ggsave(output_path, plot=last_plot(), device="pdf", path=NULL, scale=1, width=width, height=height, units=c("cm"), dpi=500, limitsize=FALSE)
}
...
for (a in approaches_to_use){
  for (c in curation_subsets_to_use){
    ...
    output_path = paste(output_dir,a,"_curated_",tolower(as.character(c)),".pdf", sep="")
