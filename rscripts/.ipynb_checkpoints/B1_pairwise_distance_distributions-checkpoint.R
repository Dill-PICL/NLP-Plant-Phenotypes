...
#   input_name <- list.files(pattern = "stacked_histograms.csv")
#   input_path <- paste(file_path_composition_files, "/", input_name, sep = "")
# }
# output_dir <- paste(parent_directory, "/figs/pairwise_distributions/", sep = "")


# The input and output files that this script uses and creates.
input_path <- "/work/triffid/prasanth/reorganizing-irb-scripts/plant-phenotypes-nlp/outputs/stacked_05_02_2023_h14m52s44_8825/stacked_histograms.csv"
...
make_and_save_figure <- function(df, output_path){
  ...
  ggsave(output_path, plot=last_plot(), device="pdf", path=NULL, scale=1, width=15, height=6, units=c("cm"), dpi=500, limitsize=FALSE)
}
