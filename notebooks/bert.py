...
# Creating and parametrizing the small BERT architecture.
...
def vectorize_with_bert(text, model, tokenizer, method="sum", layers=4):
    ...
    #sentence is input simultaneously. The vectors for the layers can concatenated or summed 
    ...
    # Check to make sure at least one token was found with an embedding to use as a the 
